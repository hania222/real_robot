...
// if later switch to X4 (CHANGE on both channels), multiply by 4 instead of 2 → 2786.
...
//
// ENCODER: TQ42-775 = 7 PPR × 2 edges (CHANGE mode) × 99.5 gear ratio
//                         = 1393 counts/rev
...
//    ( these gains are high because input is m/s but output is 0-255 PWM)
...
//  Step 3 —  Kd only if overshoot is still visible after Ki is set
...
//  will Change them via /pid_gains topic from rqt during testing, then
//  write the final working values back here when found
// ═══════════════════════════════════════════════════════
